...
namespace {
	ofstream take_log("data_generation2.log");
	ofstream take_logm("data_generation2m.log");
	ofstream take_coef("data_generation2_coef.log");
}
...
void loop(void)
{
	...
	rep(i, frame - 1) {
		...
		if (i >= delayk) {
			...
			me[i + 1] += k3 * hillA * dt;
			//lはactivateされた分だけ減る
			lmC[i + 1] += l * hillB * dt - act * lmC[i] * taRNA[i] * dt;
			lmE[i + 1] += l * hillD * dt - act * lmE[i] * taRNA[i] * dt;
			lmA[i + 1] += l * hillF * dt - act * lmA[i] * taRNA[i] * dt;
		...
		if (i >= (int)delayr) {
			...
			//taRNAはatcとくっついてはなれないので、減少したとみなせる
			taRNA[i + 1] += rt * mtaRNA[i - (int)delayr + 1] * dt - atc * (lmA[i] + lmC[i] + lmE[i]) * taRNA[i] * dt;
	...
	fprintf(gp, "set terminal postscript color\n"
				"set output sprintf(\"generetion2_no%d.ps\")\n"
				"p \"data_generation2.log\" u 0:1 with lines t \"No.%d\"\n"
			    "rep \"data_generation2.log\" u 0:2 with lines\n"
			    "rep \"data_generation2.log\" u 0:3 with lines\n"
			    "rep \"data_generation2.log\" u 0:4 with lines\n"
			    "rep \"data_generation2.log\" u 0:5 with lines\n"
			    "rep \"data_generation2.log\" u 0:6 with lines\n", numberOfData, numberOfData);
	pclose(gp);
	if (remove("data_generation2.log")) cout << "Fail to Remove" << endl;
}
