...
namespace {
	ofstream take_log("data_generation2_lock.log");
	ofstream take_logm("data_generation2m_lock.log");
	ofstream take_coef("data_generation2_coef_lock.log");
}
...
void loop(void)
{
	...
	rep(i, frame - 1) {
		...
		//転写
		if (i >= delayk) {
			double hilla = a[i - (int)delayk + 1] / (K3 + a[i - (int)delayk + 1]);
			double hillc = c[i - (int)delayk + 1] / (K3 + c[i - (int)delayk + 1]);
			double hille = e[i - (int)delayk + 1] / (K3 + e[i - (int)delayk + 1]);
		...
		//翻訳
		if (i >= (int)delayr) {
			double hillt = taRNA[i - (int)delayr + 1] / (Kt + taRNA[i - (int)delayr + 1]);
			...
			A[i + 1] += rl * hillt * amA[i - (int)delayr + 1] * dt;
			C[i + 1] += rl * hillt * amC[i - (int)delayr + 1] * dt;
			E[i + 1] += rl * hillt * amE[i - (int)delayr + 1] * dt;
		...
		take_logm << mA[i + 1] << " " << mB[i + 1] << " " << mC[i + 1] << " " << mD[i + 1] << " " << mE[i + 1] << " " << mF[i + 1] << " " << ma[i + 1] << " " << mc[i + 1] << " " << me[i + 1] << " " << mGFP[i + 1] << " " << mRFP[i + 1] << " " << amA[i + 1] << " " << amC[i + 1] << " " << amE[i + 1] << endl;
	...
	fprintf(gp, 
				...
				// "set output sprintf(\"generetion2_lock_no%d.ps\")\n"
				"set xrange[%d:%d]\n"
				"k = %d\n"
				"p \"data_generation2_lock.log\" u (($0) - k):1 with lines t \"A\"\n"
			    "rep \"data_generation2_lock.log\" u (($0) - k):2 with lines t \"B\"\n"
			    "rep \"data_generation2_lock.log\" u (($0) - k):3 with lines t \"C\"\n"
			    "rep \"data_generation2_lock.log\" u (($0) - k):4 with lines t \"D\"\n"
			    "rep \"data_generation2_lock.log\" u (($0) - k):5 with lines t \"E\"\n"
			    "rep \"data_generation2_lock.log\" u (($0) - k):6 with lines t \"F\"\n", /*numberOfData,*/ 0, frame, (numberOfData - 1) * frame);
	fprintf(gp, 
				"set terminal postscript color\n"
				"set output sprintf(\"generation2_lock_GFP_no%d.ps\")\n"
				"set xrange[%d:%d]\n"
				"k = %d\n"
				"p \"data_generation2_lock.log\" u (($0) - k):10 with lines t \"GFP\"\n"
			    "rep \"data_generation2_lock.log\" u (($0) - k):11  with lines t \"RFP\"\n", numberOfData, 0, frame, (numberOfData - 1) * frame);
	pclose(gp);
}
